...
namespace PremierLotto.Game
{
    public static class TournamentManager
    {
        public static void RunTournament(
            ...
            JackpotRolloverManager rolloverManager)
        {
            Console.Clear();
            ...
            while (Console.KeyAvailable) Console.ReadKey(true);
            Console.ReadKey(true);
            decimal carriedIn = rolloverManager.ClaimCarriedAmount(settings.Level.ToString());
                if (carriedIn > 0)
                {
                    poolManager.AddExternalAmount(carriedIn);
                    $"🎉 A rollover jackpot of ₦{carriedIn:N2} from a previous {settings.Level.ToString()} game has been added to this pool!".WriteColored(ConsoleColor.Magenta);
                    Console.ReadKey();
                }

            for (int round = 1; round <= settings.Config.NumberOfRounds; round++)
            {
                Dictionary<Player, int> roundMatches = new Dictionary<Player, int>();
                foreach (var player in activePlayers)
                {
                    Console.Clear();
                    $"============ ROUND {round} OF {settings.Config.NumberOfRounds} ============".WriteCentered(ConsoleColor.Yellow);
            ...
            else
            {
                ...
                if (settings.Level != GameLevel.Easy && winners.Count > 1)
                {
                ...
                history.AppendTournamentLog(settings.Level.ToString(), totalJackpotPool, activePlayers, winTracker);
                poolManager.ResetPool();
            }
        ...
        private static Player RunSuddenDeath(List<Player> tiedPlayers, GameSettings settings, InputHandler input, Validation validator)
        {
            ...
            while (currentCandidates.Count > 1)
            {
                ...
                currentCandidates = roundScores.Where(x => x.Value == maxMatches).Select(x => x.Key).ToList();
            }
